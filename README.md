...
### 第三阶段：文件管理 ✅
- ✅ 自动重命名文件：`[日期]_[第一作者]_[论文标题].pdf`
- ✅ 管理面板展示所有已下载论文
- ✅ 表格视图显示论文信息（标题、作者、日期、下载时间等）
- ✅ 本地存储论文记录
...
1. **网络连接**：插件需要从CDN加载pdf.js库，确保网络连接正常
2. **跨域限制**：某些PDF链接可能因为CORS限制无法直接解析，会回退到基础下载模式
3. **文件权限**：插件需要下载权限，首次使用时浏览器会提示
4. **存储空间**：论文记录存储在浏览器本地，不会上传到服务器
